...
/**
 * LuaTool - updated to:
 * - avoid duplicate character input (printable from keyTyped only),
 * - provide controlled repeat for control keys (backspace/arrow/delete),
 * - expose precise font measurement helpers (text_width, text_width_sub, font_height),
 * - provide mouse() mapping (returns top-origin y) and set_gutter_left binding so Lua can drive a dynamic gutter.
 *
 * Important change:
 * - mouse().y now returns a top-origin coordinate (0 at top of editor area), which matches the renderer.lua
 *   calculations that use top-based math (content_top - m.y).
 * - For backwards compatibility, mouse() also returns "y_bottom" as the bottom-origin value.
 */
public class LuaTool implements ToolModule {
    ...
    private void bindDrawingFunctions() {
        ...
        // mouse() mapping into editor coordinate space: returns top-origin y (0 at top of editor area)
        // and also returns y_bottom for backward compatibility (bottom-origin).
        globals.set("mouse", new ZeroArgFunction() {
            @Override public LuaValue call() {
                org.luaj.vm2.LuaTable t = new org.luaj.vm2.LuaTable();
                int sx = Gdx.input.getX();
                int sy_top = Gdx.input.getY(); // libGDX gives Y from top for desktop backends
                ...
                if (areaW <= 0 || areaH <= 0) {
                    // Provide raw screen coords (top-origin)
                    t.set("x", LuaValue.valueOf(sx));
                    t.set("y", LuaValue.valueOf(Gdx.graphics.getHeight() - sy_top));
                    // y here remains top-origin for compatibility with callers expecting top-based math
                } else {
                    // local pixel coords inside drawn area (screen-space from top)
                    float localX = sx - areaX - gutter;
                    float localYFromTop = sy_top - areaTopY - gutterTop; // top-origin inside area
                    // clamp
                    ...
                    // convert to FBO / texture space (scale)
                    float tx = (areaW > 0) ? (localX * ((float) fboWidth / (float) areaW)) : localX;
                    float ty_top = (areaH > 0) ? (localYFromTop * ((float) fboHeight / (float) areaH)) : localYFromTop;
                    // also compute bottom-origin for compatibility
                    float ty_bottom = (areaH > 0) ? ((areaH - localYFromTop) * ((float) fboHeight / (float) areaH)) : (areaH - localYFromTop);

                    t.set("x", LuaValue.valueOf(Math.round(tx)));
                    // IMPORTANT: return y as TOP-ORIGIN (matches renderer.lua math using content_top - m.y)
                    t.set("y", LuaValue.valueOf(Math.round(ty_top)));
                    // provide bottom-origin as well for any legacy code
                    t.set("y_bottom", LuaValue.valueOf(Math.round(ty_bottom)));
                }
